...
public class DatabaseProvider extends ContentProvider {
    ...
    private static final int REQUESTS_TABLE = 1;

    private static final UriMatcher URI_MATCHER;

    static {
        URI_MATCHER = new UriMatcher(UriMatcher.NO_MATCH);
        URI_MATCHER.addURI(SqliteHelper.CONTENT_AUTHORITY, RequestItemContract.DatabaseRequests.TABLE_NAME, REQUESTS_TABLE);
    }
    ...
    public String getType(@NonNull Uri uri) {
        switch (URI_MATCHER.match(uri)) {
            case REQUESTS_TABLE:
                return RequestItemContract.DatabaseRequests.TABLE_NAME;
            default:
                return "";
        }
    }
